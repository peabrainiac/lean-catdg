...
class GrothendieckTopology.IsConcreteSite (FC : outParam (C → C → Type v))
    {CC : outParam (C → Type v)} [outParam (∀ X Y, FunLike (FC X Y) (CC X) (CC Y))]
    (J : GrothendieckTopology C) extends HasTerminal C, ConcreteCategory C FC where
  ...
  forgetNatIsoCoyoneda_apply {X : C} {x : ToType X} :
    (hom <| forgetNatIsoCoyoneda.hom.app X x : ToType (⊤_ C) ⟶ ToType X) = fun _ ↦ x
  /-- All covering sieves are jointly surjective. -/
  isJointlySurjective_of_mem {X : C} {S : Sieve X} (hS : S ∈ J X) : S.arrows.IsJointlySurjective

variable {FC : C → C → Type v} {CC : C → Type v} [∀ X Y, FunLike (FC X Y) (CC X) (CC Y)]

namespace GrothendieckTopology.IsConcreteSite

lemma coyoneda_obj_terminal_faithful (J : GrothendieckTopology C) [J.IsConcreteSite FC] :
...
noncomputable instance instUniqueTerminal
    (J : GrothendieckTopology C) [J.IsConcreteSite FC] : Unique (ToType (⊤_ C)) :=
  ((forgetNatIsoCoyoneda (J := J)).app (⊤_ C)).toEquiv.unique (β := ⊤_ C ⟶ ⊤_ C)

/-- Every concrete site is also a local site. -/
instance isLocalSite (J : GrothendieckTopology C) [J.IsConcreteSite FC] : J.IsLocalSite where
  eq_top_of_mem S hS := (S.id_mem_iff_eq_top).1 <| by
    let ⟨Y, f, hf, y, hfy⟩ := IsConcreteSite.isJointlySurjective_of_mem hS default
    let y' : ⊤_ C ⟶ Y := (forgetNatIsoCoyoneda (J := J)).hom.app Y y
    convert S.downward_closed hf y'
    exact Subsingleton.elim _ _

/-- On concrete sites, covering sieves contain all morphisms from the terminal object. -/
lemma from_terminal_mem_of_mem (J : GrothendieckTopology C) [J.IsConcreteSite FC] {X : C}
    {S : Sieve X} (hS : S ∈ J X) (f : ⊤_ C ⟶ X) : S f := by
  ...
  apply (forget C).map_injective; ext (x : ToType (⊤_ C))
...
lemma isSeparated_yoneda_obj (J : GrothendieckTopology C) [J.IsConcreteSite FC] (X : C) :
...
at least separated. -/
lemma isSeparated_of_isRepresentable (J : GrothendieckTopology C) [J.IsConcreteSite FC]
...
variable (J : GrothendieckTopology C) [J.IsConcreteSite FC]
