...
functor (which is proven in `CatDG.Diffeology.DiffSp`); on the level of individual types,
...
lemma dTop_generateFrom_eq_iSup {X : Type*} {g : Set ((n : ℕ) × (Eucl n → X))} :
  ...
  refine le_antisymm ((dTop_mono ?_).trans dTop_continuousDiffeology_le) <|
    iSup₂_le fun p hp ↦ (isPlot_generateFrom_of_mem hp).continuous.coinduced_le
