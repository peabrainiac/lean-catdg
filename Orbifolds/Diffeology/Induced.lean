...
lemma DiffeologicalSpace.coinduced_eq_generateFrom {X Y : Type*} [dX : DiffeologicalSpace X]
    {f : X → Y} : dX.coinduced f = generateFrom {p | ∃ q, IsPlot q ∧ p.2 = f ∘ q} :=
  le_antisymm (DSmooth.coinduced_le fun _ p hp ↦ isPlot_generatedFrom_of_mem ⟨p, hp, rfl⟩)
...
theorem dTop_coinduced_comm {X Y : Type*} {dX : DiffeologicalSpace X} {f : X → Y} :
  ...
  have hf : DSmooth f := by rw [dsmooth_iff_coinduced_le]
  refine le_antisymm ?_ hf.continuous.coinduced_le
  rw [DiffeologicalSpace.coinduced_eq_generateFrom, dTop_generateFrom_eq_iSup]
