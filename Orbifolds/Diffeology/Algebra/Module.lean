...
lemma LinearMap.isPlot (X : Type*) [AddCommGroup X] [Module ℝ X] [DiffeologicalSpace X]
    [DiffeologicalModule ℝ X] {n : ℕ} (p : Eucl n →ₗ[ℝ] X) : IsPlot p := by
...
instance {X : Type*} [AddCommGroup X] [Module R X] [DiffeologicalSpace X]
    ...
    DiffeologicalModule R (X × Y) where

/-- Arbitrary products of diffeological modules are diffeological modules. -/
instance Pi.diffeologicalModule {ι : Type*} {X : ι → Type*} [∀ i, AddCommGroup (X i)]
    [∀ i, Module R (X i)] [∀ i, DiffeologicalSpace (X i)] [∀ i, DiffeologicalModule R (X i)] :
    DiffeologicalModule R (∀ i, X i) where
...
theorem diffeologicalModule_induced {X : Type*} [AddCommGroup X] [Module R X] {Y : Type*}
    ...
    @DiffeologicalModule R _ _ _ X (DiffeologicalSpace.induced f ‹_›) _ _ :=
...
theorem diffeologicalModule_iInf {X : Type*} [AddCommGroup X] [Module R X] {ι : Type*}
...
lemma fineDiffeology_le (R : Type*) [DiffeologicalSpace R] [Ring R] [DiffeologicalRing R]
    (X : Type*) [AddCommGroup X] [Module R X] [d : DiffeologicalSpace X]
    [DiffeologicalModule R X] : fineDiffeology R X ≤ d :=
  ModuleDiffeology.toDiffeologicalSpace_le.2
    (bot_le : (⊥ : ModuleDiffeology R X) ≤ ⟨d, inferInstance⟩)
...
lemma fineDiffeology_eq_euclideanDiffeology (X : Type*) [NormedAddCommGroup X] [NormedSpace ℝ X]
  ...
  refine le_antisymm (@fineDiffeology_le ℝ _ _ _ X _ _ (_) _) ?_
