...
instance (X : Type*) [NormedAddCommGroup X] [NormedSpace ℝ X] [DiffeologicalSpace X]
    [ContDiffCompatible X] : DSmoothSMul ℝ X where
...
example {R : Type*} [Ring R] [DiffeologicalSpace R] [TopologicalSpace R] [DTopCompatible R]
    ...
    [LocallyCompactSpace X] : ContinuousSMul R X ∧ IsTopologicalAddGroup X :=
...
def toAddGroupDiffeology (d : ModuleDiffeology R X) : AddGroupDiffeology X :=
  ⟨d.toDiffeologicalSpace, d.toDiffeologicalAddGroup⟩
...
def gci_toDiffeologicalSpace :
  ...
  choice d hd := ⟨d, le_antisymm (le_sInf fun _ h ↦ h.1) hd ▸
    (reflector d).toDiffeologicalAddGroup, le_antisymm (le_sInf fun _ h ↦ h.1) hd ▸
    (reflector d).toDSmoothSMul⟩
...
lemma IsFineDiffeology.diffeologicalAddGroup (R : Type*) [DiffeologicalSpace R] [Ring R]
  ...
  eq_fineDiffeology R X ▸ (⊥ : ModuleDiffeology R X).toDiffeologicalAddGroup

/-- Since real diffeological vector spaces are the most important special case of diffeological
modules, we register at least `IsFineDiffeology.diffeologicalAddGroup ℝ X` as an instance. -/
instance IsFineDiffeology.diffeologicalAddGroupReal (X : Type*) [AddCommGroup X] [Module ℝ X]
    [DiffeologicalSpace X] [IsFineDiffeology ℝ X] : DiffeologicalAddGroup X :=
  IsFineDiffeology.diffeologicalAddGroup ℝ X
...
instance IsFineDiffeology.dsmoothSMul (R : Type*) [DiffeologicalSpace R] [Ring R]
  ...
  eq_fineDiffeology R X ▸ (⊥ : ModuleDiffeology R X).toDSmoothSMul
...
lemma fineDiffeology_le (R : Type*) [DiffeologicalSpace R] [Ring R] [DiffeologicalRing R]
    (X : Type*) [AddCommGroup X] [Module R X] [d : DiffeologicalSpace X]
    [DiffeologicalAddGroup X] [DSmoothSMul R X]: fineDiffeology R X ≤ d :=
  ModuleDiffeology.toDiffeologicalSpace_le.2
    (bot_le : (⊥ : ModuleDiffeology R X) ≤ ⟨d, inferInstance, inferInstance⟩)
...
lemma LinearMap.dsmooth (R : Type*) [DiffeologicalSpace R] [Ring R] [DiffeologicalRing R]
    ...
    [DSmoothSMul R Y] (f : X →ₗ[R] Y) : DSmooth f :=
...
instance {X : Type*} [NormedAddCommGroup X] [NormedSpace ℝ X] [FiniteDimensional ℝ X]
    [DiffeologicalSpace X] [IsFineDiffeology ℝ X] : ContDiffCompatible X := by
  rw [eq_fineDiffeology ℝ X, fineDiffeology_eq_euclideanDiffeology X]
  infer_instance

instance {X : Type*} [NormedAddCommGroup X] [NormedSpace ℝ X] [FiniteDimensional ℝ X]
    [DiffeologicalSpace X] [ContDiffCompatible X] : IsFineDiffeology ℝ X := by
